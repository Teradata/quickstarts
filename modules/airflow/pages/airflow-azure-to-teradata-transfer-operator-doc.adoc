...
* Create an https://docs.microsoft.com/en-us/azure/storage/common/storage-quickstart-create-account?tabs=azure-portal[Azure storage account] with name `azuretestquickstart`
* Create a https://learn.microsoft.com/en-us/azure/storage/blobs/blob-containers-portal[blob container] with name `quickstartdemo` under Azure storage account `azuretestquickstart`
* https://learn.microsoft.com/en-us/azure/storage/blobs/storage-quickstart-blobs-portal[Upload] CSV/JSON/Parquest format files to `quickstartdemo` blob container.
...
+
2. Install Apache Airflow Teradata provider package and Microsoft Azure provider package.
+
[source, bash]
----
...
== Create Teradata Authorization
NOTE: A database client connected to Vantage is needed to execute SQL statements. https://downloads.teradata.com/download/tools/teradata-studio-express[Teradata Studio Express] or https://quickstarts.teradata.com/other-integrations/configure-a-teradata-vantage-connection-in-dbeaver.html[dbeaver] can be used for this purpose.
...
1. Switch to virtual environment where Apache Airflow was installed at <<Install Apache Airflow>>
+
[source, bash]
----
source airflow_env/bin/activate
...
Dags in airflow are defined as python files. The dag below transfer CSV/JSON/Parquet data from Microsoft Azure Blob Storage to Teradata Vantage instance.Copy the python code below and save it as `airflow-azure-to-teradata-transfer-operator-demo.py` under the directory $AIRFLOW_HOME/dags.

Refer https://airflow.apache.org/docs/apache-airflow-providers-teradata/stable/_api/airflow/providers/teradata/transfers/azure_blob_to_teradata/index.html[AzureBlobStorageToTeradataOperator] for more information on `Azure Blob Storage to Teradata` Transfer Operator.
