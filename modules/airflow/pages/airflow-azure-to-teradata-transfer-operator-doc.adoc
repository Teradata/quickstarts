...
Dags in airflow are defined as python files. The dag below transfer CSV data from Teradata-supplied public blob containers to Teradata Vantage instance.Copy the python code below and save it as `airflow-azure-to-teradata-transfer-operator-demo.py` under the directory $AIRFLOW_HOME/dags.
...
* Create Teradata Authorization object with Azure Blob Storage Account and Account Secret Key
+
[source, teradata-sql]
----
