...
Dags in airflow are defined as python files. The dag below transfer data from Teradata-supplied public buckets to Teradata Vantage instance.Copy the python code below and save it as `airflow-aws-to-teradata-transfer-operator-demo.py` under the directory $AIRFLOW_HOME/dags.
...
To successfully transfer data from Private Amazon S3 bucket to a Teradata instance, the following prerequisites are necessary.
...
* Create Teradata Authorization object with AWS Account Key and Account Secret Key
+
[source, teradata-sql]
----
