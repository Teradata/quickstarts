...
* Create a https://docs.aws.amazon.com/AmazonS3/latest/userguide/creating-bucket.html[S3 bucket] with name `awstestquickstart`
* https://docs.aws.amazon.com/AmazonS3/latest/userguide/uploading-an-object-bucket.html[Upload] CSV/JSON/Parquest format files to `awstestquickstart` S3 bucket.
...
+
2. Install Apache Airflow Teradata provider package and Amazon provider package.
+
[source, bash]
----
...
Dags in airflow are defined as python files. The dag below transfer CSV/JSON/Parquet data from Amazon S3 to Teradata Vantage instance.Copy the python code below and save it as `airflow-aws-to-teradata-transfer-operator-demo.py` under the directory $AIRFLOW_HOME/dags.
