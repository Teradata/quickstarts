...
include::ROOT:partial$understanding.regulus.adoc[]

This document outlines the steps for installing and configuring Regulus using Docker. To use Regulus with Workspaces CLI, see xref::using.regulus.workspacecli.adoc[].
...
+
image::regulus/regulus.workspaces.setting.png[]
...
* Workspaces CLI: See xref::using.regulus.workspacecli.adoc[].
...
* Get started with Regulus by running a simple workflow. See xref::getting.started.regulus.adoc[].

* Interested in learning how Regulus can help you with real-life use cases? See <GitHub Sample link>.
