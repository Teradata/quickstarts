...
This tutorial explains the architecture of Teradata Vantage and the underlying concepts. 

Teradata's architecture is designed around the Massively Parallel Processing (MPP) architecture, which enables high-performance data processing and analytics. The major architectural components of Teradata include the Parsing Engine, Message Passing Layer (MPL), Access Module Processors (AMPs), and Virtual Disks (Vdisks).
...
The Teradata Vantage architecture consists of below components.

=== Parsing Engines (PE)
When a SQL query is run in Teradata, it first reaches the Parsing Engine. The functions of the PE are:
