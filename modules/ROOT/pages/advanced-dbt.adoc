...
If you are new to data engineering with dbt we recommend that you start with our introductory project.[Add link]
...
* A database client for running database commands, an example of configuration of one such client is presented here https://quickstarts.teradata.com/other-integrations/configure-a-teradata-vantage-connection-in-dbeaver.html.
...
To achieve this objective we provide public datasets available in Google Cload Platform, and scripts to load those datasets into your mock data warehouse. + 

1. Create or select a working database. The dbt profile in the project points to a database called `teddy_retailers`. You can change the `schema` value to point to an existing database in your Teradata Vantage instance or you can create the `teddy_retailers` database:
+
[source, teradata-sql]
----
...
Create file `$HOME/.dbt/profiles.yml` with the following content. Adjust `<host>`, `<user>`, `<password>` to match your Teradata Vantage instance.
...
With dbt we transform takes these source data tables to built the following dimensional model, which is more suitable for analytics tools:
...
The `schema.yml` file located in the project's models directory root defines the sources for our models. These sources correspond to the data that we loaded from GCP using our SQL scripts.
...
In the `all_order_products` model, we have incorporated an assertion with the assistance of a macro as a test to ensure that the resulting model contains a distinct combination of `order_id` and `product_id`. 
This combination represents the most recent quantity of products per order.
...
We create the dimensional model with baseline data by running dbt:
[source, bash]
----
dbt run
----

Our core and dimensional models will be created now with the baseline data.
...
=== Running testing queries

Testing business intelligence queries are available in the `references/query` path of the project.

=== Mocking the ELT process

The scripts to load updates into the source data set are available in the `references/inserts/update_data.sql` path of the project.

After updating the data sources you can run all the steps above, run dbt, test the data, run testing queries, to visualize the variations and incremental updates.

== Summary

This tutorial demonstrated how to use advanced dbt concepts with Teradata Vantage. 
The sample project takes source data and produces a dimensional data mart. 
We apply the following advanced dbt concepts, incremental materializations, utility macros, teradata modifiers.

== For any questions.
https://support.teradata.com/community?id=community_forum&sys_id=353d17d44fed7f00dcf7a6118110c7b5&utm_source=teradata_university&utm_medium=TVUG&utm_campaign=advanced_dbt
