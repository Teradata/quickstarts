...
gcloud compute instances create teradata-vantage-express `
  ...
  --create-disk=boot=yes,device-name=ve-disk,image-family=ubuntu-2004-lts,size=70,type=pd-balanced `
...
----
apt update
DEBIAN_FRONTEND=noninteractive apt-get install -qq p7zip-full p7zip-rar virtualbox < /dev/null > /dev/null
----
