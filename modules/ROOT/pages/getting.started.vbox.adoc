...
. We will now create a VirtualBox VM that uses the downloaded image:
+
[source, bash]
----
...
vboxmanage modifyvm $VM_NAME --ioapic on --memory 8096 --vram 128 --nic1 nat --graphicscontroller vmsvga --usb on --mouse usbtablet --clipboard-mode bidirectional
...
vboxmanage modifyvm $VM_NAME --natpf1 "guestssh,tcp,,4422,,22"
