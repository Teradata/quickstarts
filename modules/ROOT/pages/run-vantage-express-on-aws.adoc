...
.Cloud charges
====
Vantage Express is distributed as a virtual machine image. The how-to uses the EC2 `c5n.metal` instance type. It's a bare metal instance that is very expensive. At the time of writing this how-to, the instance cost over $3/h. Still, it's the cheapest instance that supports guest virtual machines.

If do not wish to pay for cloud usage, you can install Vantage Express locally using link:getting.started.vmware.adoc[VMware], link:getting.started.vbox[VirtualBox], or xref:getting.started.utm.adoc[UTM]. You can also try link:vantage-express.gcp.adoc[GCP] and link:run-vantage-express-on-microsoft-azure.adoc[Azure] which support nested virtualization and can run Vantage Express on cheap VM's.
====
