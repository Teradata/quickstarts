...
If you want a cheaper option, try link:vantage-express.gcp.adoc[GCP] and link:run-vantage-express-on-microsoft-azure.adoc[Azure] which support nested virtualization and can run Vantage Express on cheap VM's.

If you do not wish to pay for cloud usage at all, install Vantage Express locally using link:getting.started.vmware.adoc[VMware], link:getting.started.vbox[VirtualBox], or xref:getting.started.utm.adoc[UTM].
====
