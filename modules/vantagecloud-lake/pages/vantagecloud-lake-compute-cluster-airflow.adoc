...
3. Install the Apache Airflow Teradata provider stable version from PyPI repository.
+
[source, bash]
----
...
1. Clone the jaffle_shop dbt demo repository and cd into the project directory:
+
[source, bash]
----
...
cd jaffle_shop
----
2. Create `profiles.yml` file. Add the following config to `profile.yml` file. Adjust `<host>` to match your Teradata VantageCloud Lake instance.
+
[source, yaml]
----
jaffle_shop:
...
2. Click on + to define new connection to the Teradata VantageCloud Lake instance.
3. Define a new connection with the Teradata VantageCloud Lake instance details. Assign the id `teradata_lake`
...
Once the dag file is copied to $AIRFLOW_HOME/files/dags, Apache Airflow loads the dag to the airflow UI.
