= Automate ELT workflows with Terraform and Airbyte on Teradata Vantage 
:experimental:
:page-author: Janeth Graziani
:page-email: Janeth.graziani@teradata.com
:page-revdate: February 24, 2024
:description: Use Airbyte's Terraform provider to automate and version-control your Airbyte ELT workflows on Teradata Vantage.
...
In this Quickstart, you'll learn to use Terraform to automate Airbyte ELT workflows on Teradata Vantage. Instead of manual configurations through the WebUI, we'll use code to manage Airbyte resources. The provided example illustrates a basic ELT workflow from Google Sheets to Teradata Vantage using Airbyte's Terraform provider.
...
== Prerequisites
* https://airbyte.com/connectors/teradata-vantage[Airbyte Cloud Account]. Start with a 14-day free trial that begins after your first successful sync.
...
* Source Data. For demonstration purposes we will use a link:https://docs.google.com/spreadsheets/d/1XNBYUw3p7xG6ptfwjChqZ-dNXbTuVwPi7ToQfYKgJIE/edit#gid=0[sample google sheets,  window="_blank"]. Make a copy of it into your Google worspace. 
...
Prepare your environment by creating a directory for your Terraform configuration and initialize two files: `main.tf` and `variable.tf`.
...
Define your data source, destination and connection within the `main.tf` file. Open your newly created `main.tf` file in Visual Studio Code or your favorite code editor.

- If using Visual Studio Code you can install https://marketplace.visualstudio.com/items?itemName=HashiCorp.terraform[HashiCorp Terraform Extensions] to add autocompletion and syntax highlighting. You can also add https://marketplace.visualstudio.com/items?itemName=4ops.terraform[Terraform by Anton Kulikov] for configuration language support. 

image::{dir}/extensions.png[Terraform Extensions on Visual Studio Code, align="center" width=75%]

Populate your main.tf file with the template provided, adjusting for your specific source and destination. Detailed information and configurations can be found in the Airbyte API reference documentation for https://reference.airbyte.com/reference/createsource[creating Sources].  
...
provider "airbyte" {
  // If running on Airbyte Cloud, generate & save your API key from https://portal.airbyte.com
...
Note that this example uses cron expression for scheduling the data transfer to run every 15 minutes past the hour. 

In our `main.tf` file we reference variables which are held in the `variables.tf` file, including the API key, workspace ID, Google Sheet id, Google private key and Teradata Vantage credentials. Copy the following template into your `variables.tf` file and populate with your configuration values in the `default` key.

== Configuring the variables.tf file

[source, bash]
----
#log in to https://portal.airbyte.com generate, save and populate the variable with your API key
variable "api_key" {
    type = string
    default = ""
}
#workspace_id is found in the url to your Airbyte Cloud account https://cloud.airbyte.com/workspaces/<workspace_id>/settings/dbt-cloud 
...
For this example you will a plan for 3 resources created:
