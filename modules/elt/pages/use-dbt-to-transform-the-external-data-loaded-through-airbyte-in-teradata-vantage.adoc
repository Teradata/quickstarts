= Use dbt to transform external data loaded through Airbyte in Teradata Vantage
:experimental:
...
In this configuration, `schema` stands for the database that contains the Airbyte raw Jaffle Shop dataset in JSON format. In this case we define the `schema` with the same default `schema` that we defined in Airbyte `airbyte_jaffle_shop`.
...
This dbt project performs two types of transformations.

* First, it transforms the raw data (in JSON format), loaded from Google Sheets via Airbyte, into staging models materialized through views. At this stage the data is normalized. 
...
Data loaded through Airbyte though is represented as raw JSON strings. From these raw data we are creating normalized views. We perform this task through the following staging models. 
The `stg_customers` model creates the normalized view for `customers` from `_airbyte_raw_customers` table. Similarly the `stg_orders` model creates the normalized view for `orders` from `_airbyte_raw_orders`,  and, finally the `stg_payments` model creates the normalized view for `payments` from `_airbyte_raw_payments`. 
...
* In the second step, dbt creates `dim_customers` and `fct_orders` tables. These constitute the dimensional model tables that we want to expose to our BI tool. You will find the definitions of these tables in `./models/marts/core`.
