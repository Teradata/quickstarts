...
* Docker Compose to run the link:https://github.com/airbytehq/airbyte[Airbyte Open Source] locally. Docker Compose comes with Docker Desktop. Please refer to link:https://docs.docker.com/compose/install/[docker docs,  window="_blank"] for more details. 
...
* link:https://support.google.com/googleapi/answer/6158841?hl=en[Google Cloud Platform API enabled for your personal or organizational account,  window="_blank"]. You’ll need to authenticate your Google account via OAuth or via Service Account Key Authenticator.
...
* The default schema name is optional, however, you can provide a more appropriate name to your specific context. Here we have provided `gsheet_airbyte_td`.
...
For more details please refer link:https://docs.airbyte.com/understanding-airbyte/namespaces/[Airbyte Namespace., window="_blank"]


image::{dir}/namespaces_in_destination.png[Namespaces in the destination]


In our example the destination is a database, so the namespace is the default schema `gsheet_airbyte_td` we defined when we configured the destination. The stream name is a table that is mirroring the name of the spreadsheet in the source, which is `sample_employee_payrate` in this case. + 
Other type of sources and destinations might have a different layout. In this example, Google sheets as source does not support a namespace. Since we are using the single spreadsheet connector, it only supports one stream (the active spreadsheet).
...
Next, you can go to the link:https://clearscape.teradata.com/dashboard[ClearScape Analytics Experience, window="_blank"] and run a Jupyter notebook for Teradata SQL, to verify if the database `gsheet_airbyte_td`, streams (tables) and complete data is present.
