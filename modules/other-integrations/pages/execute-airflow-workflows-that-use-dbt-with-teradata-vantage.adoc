...
+
[source, teradatasql, id="create_databases", role="content-editable emits-gtm-events"]
----
...
2. Examine the files:
  ...
  * `discover_dag.txt` - an example on how to load various types of data files (CSV, Parquet, JSON). The source code file contains comments that explain what the program does and how to use it. This example relies on `variables.json` file. TODO: Does `variables.json` need to be adjusted?
