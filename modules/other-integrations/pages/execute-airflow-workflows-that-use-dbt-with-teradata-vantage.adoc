...
* `discover_dag.py` - an example on how to load various types of data files (CSV, Parquet, JSON). The source code file contains comments that explain what the program does and how to use it. This example relies on `variables.json` file. The file needs to be imported into Airflow.
...
4. Import `variables.json` file as a variable file into Airflow by navigating to `Admin -> Variables` menu and adjust the variables for your environment:
+
