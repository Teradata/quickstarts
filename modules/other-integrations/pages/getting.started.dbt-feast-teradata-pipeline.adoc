...
* dbt library installed locally. See link:https://quickstarts.teradata.com/dbt.html[dbt Installation instructions]

== Objective
To create a data pipeline with Teradata Vantage as a source, perform data transformation on some variables in dbt. The principle transformation of data we do in dbt is the one-hot encoding of several columns like gender, marital status, state code, etc. On top of that, the account type column data will be transformed by performing aggregation operations on a couple of columns. All of this together generates our desired Analytic_dataset with transformed data. Then we use the transformed dataset output as input in FEAST to store features that can be used to generate a training dataset for models.
...
----
== Run dbt
Teradata Vantage has long been recognized as a database that provides superior ELT(Extract Load Transform) capabilities. dbt enhances these capabilities by making use of its efficient Analytic functions to transform data.
...
== Summary
This tutorial demonstrated how to use dbt and FEAST with Teradata Vantage. The sample project takes raw data in Teradata Vantage and produces features with dbt, which is again saved as a model in the database itself. Metadata of features that form the base to generate a training dataset for a model was then created with FEAST; all its corresponding tables that create the feature store, are also generated at runtime within the same database. This sample project gives us an idea of how to integrate these three robust platforms.
