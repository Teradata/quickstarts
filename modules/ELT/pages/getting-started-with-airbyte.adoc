...
== Prerequsites
* Access to Teradata Vantage Instance. As this will be our destination. You will need a database host, username, and password for Airbyte’s configuration.
+
...
== Launch Airbyte Opensource
* Clone the Airbyte OpenSource repository and go to the airbyte directory.
+
[source, bash]
----
...
After you login with the above credentials, it will ask you for your email and specify your preferences for product improvements.

image::{dir}/specify_preferences.png[Specify Preferences, width=75%]

For more details, please visit link:https://github.com/airbytehq/airbyte[Airbyte github]. Once the Airbyte OpenSource is launched you will see a connections dashboard. If you launched Airbyte open source for the first time, it would not show any connections.
...
* Here we are using `Service Account Key Authentication` which uses a service account key in JSON format. Toggle from the default `OAuth` to `Service Account Key Authentication`. You need to provide the JSON key in Service Account Information. You can refer to the link:https://support.google.com/googleapi/answer/6158841?hl=en[steps to activate the API] and link:https://developers.google.com/workspace/guides/create-credentials#:~:text=To%20obtain%20credentials%20for%20your%20service%20account%3A[create credentials for service account in JSON format].

* The link to the spreadsheet to pull the data from is to be added in Spreadsheet Link. By default, the row batch size is 200.

image::{dir}/configuring_source_gsheet_airbyte.png[Configuring the source in Airbyte]

[NOTE]
====
You might get a Configuration check failed error, HTTP error 403. Make sure your spreadsheet has a general view access. 
====
...
* Assuming you want to create a fresh new connection with `Teradata Vantage`, Select `Teradata Vantage` as destination type under Set up the destination section. If you already have an existing connection and want to use the same, you can select from Select an existing destination section. 
* Add the `Host`, `User`, and `Password`. This is the same as `Host`, `Username`, and `Password` that are used to create the Teradata Vantage instance in the Clearscape Analytics Environment.
...
It shows how often data should sync to destination. You can select every hour, 2 hours, 3 hours etc. Like in our case we used every 24 hours. 
...
[source, bash]
----
SET SESSION DATABASE gsheet_airbyte_td; 
----
